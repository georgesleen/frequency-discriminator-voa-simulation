...
``output/carrier_maps.png`` - two rows of four panels:
  ...
  Row 2: injected carrier change dn = n(V) - n(0) at the same voltages,
         zoomed to +/-1.5 um around the waveguide rib.
...
ZOOM_X_UM = 1.5   # um half-width for the rib-region zoom (row 2)


def _rib_patch(**kw):
    """Return a Rectangle that outlines the rib in plot coordinates (um / nm)."""
...
def main() -> None:
    ...
    cb1 = fig.colorbar(im1, ax=axes[0, :], label="n  (cm^-3)", shrink=0.85, pad=0.01)
    cb1.ax.tick_params(labelsize=8)

    # -- Row 2: dn = n(V)-n(0), rib zoom, log scale ---------------------------
    ...
    for col, (vi, V) in enumerate(zip(v_idx, PLOT_VOLTAGES)):
        ...
        if col == 0:
            ax.set_ylabel("y (nm)", fontsize=9)
            ax.text(-1.4, 190, "dn vs 0V", fontsize=8, color="white", va="top")
        ax.tick_params(labelsize=8)

    cb2 = fig.colorbar(im2, ax=axes[1, :], label="dn  (cm^-3)", shrink=0.85, pad=0.01)
